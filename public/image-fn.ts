...
const processRoundImage = async () => {
  ...
  const imgElement = document.querySelector('.round-img-modified') as HTMLImageElement;
  if (!imgElement) {
    return;
  }
...
const processCircularImage = async () => {
  ...
  const imgElement = document.querySelector('.circular-img-modified') as HTMLImageElement;
  if (!imgElement) {
    return;
  }
  imgElement.src = roundedImg.src;
