...
export const removeTilde = (text: string): string => {
  ...
  const result = characters.reduce(
    (accumulator, current) => accumulator.replace(new RegExp(current[0], 'g'), current[1]),
    text
  );
  return result;
};
 ...
 * @returns
 */
export const isEmptyString = (text: unknown): boolean =>
 ...
 * @returns
 */
export const isStringNotEmpty = (text: unknown): boolean =>
 ...
 * @returns
 */
export const stringIncludesOmitTilde = (text: string, subtext: string): boolean => {
